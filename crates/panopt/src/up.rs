...
/// Zellij's stock pane-mode (`Ctrl-p`) split keys, retargeted to the same
/// `panopt:spawn-blank-pane` pipe as Alt-N. Without this, `Ctrl-p` then
/// `n`/`d`/`r`/`s` reaches Zellij's native `NewPane`, which splits whatever
/// pane is focused - and a sidebar plugin pane sits in Normal mode where pane
/// mode is reachable, so the gesture shreds the fixed 25% sidebar column. The
/// plugin's `spawn_blank_pane` gate already refuses the pipe when a sidebar
/// pane is focused (and routes it into a viewer slot otherwise), so funneling
/// every split key through it collapses the directional split semantics into
/// the cockpit's one swap-into-slot model - the same treatment Alt-N gets.
/// `SwitchToMode "Normal"` is preserved so the keypress still leaves pane
/// mode. Pane mode is unreachable from an auto-locked content pane, so no
/// locked-block variant is needed. Each FROM is the exact text the defaults
/// dump emits; see [`NEW_PANE_BIND_FROM`] for why the cockpit pipes through
/// `zellij action pipe` rather than `MessagePlugin`.
const PANE_SPLIT_BINDS: &[(&str, &str)] = &[
    (
        r#"bind "n" { NewPane; SwitchToMode "Normal"; }"#,
        r#"bind "n" { Run "zellij" "action" "pipe" "--name" "panopt:spawn-blank-pane" "--plugin-configuration" "mode=todos" { close_on_exit true; }; SwitchToMode "Normal"; }"#,
    ),
    (
        r#"bind "d" { NewPane "Down"; SwitchToMode "Normal"; }"#,
        r#"bind "d" { Run "zellij" "action" "pipe" "--name" "panopt:spawn-blank-pane" "--plugin-configuration" "mode=todos" { close_on_exit true; }; SwitchToMode "Normal"; }"#,
    ),
    (
        r#"bind "r" { NewPane "Right"; SwitchToMode "Normal"; }"#,
        r#"bind "r" { Run "zellij" "action" "pipe" "--name" "panopt:spawn-blank-pane" "--plugin-configuration" "mode=todos" { close_on_exit true; }; SwitchToMode "Normal"; }"#,
    ),
    (
        r#"bind "s" { NewPane "stacked"; SwitchToMode "Normal"; }"#,
        r#"bind "s" { Run "zellij" "action" "pipe" "--name" "panopt:spawn-blank-pane" "--plugin-configuration" "mode=todos" { close_on_exit true; }; SwitchToMode "Normal"; }"#,
    ),
];
...
/// Retarget Zellij's pane-mode split keys (`n`/`d`/`r`/`s`) to the same
/// spawn-blank-pane pipe Alt-N uses, so they cannot split a focused sidebar
/// pane. Returns the count of binds actually rewritten; the caller warns when
/// it is short of [`PANE_SPLIT_BINDS`]'s length so the user knows a
/// reformatted config left the gate with gaps. See [`PANE_SPLIT_BINDS`].
fn retarget_pane_mode_splits(base: &str) -> (String, usize) {
    let mut out = base.to_string();
    let mut changed = 0;
    for (from, to) in PANE_SPLIT_BINDS {
...
fn render_config(copy_helper: Option<&Path>) -> Result<PathBuf> {
    ...
    let (tweaked, pane_splits_changed) = retarget_pane_mode_splits(&tweaked);
    if pane_splits_changed < PANE_SPLIT_BINDS.len() {
        eprintln!(
            "warning: only {pane_splits_changed} of {} Zellij pane-mode split \
             bindings (`n`/`d`/`r`/`s`) were found in the expected form, so \
             they could not all be gated; the missing ones will split a \
             focused sidebar pane instead of opening a viewer slot",
            PANE_SPLIT_BINDS.len()
        );
    }
    ...
    let body = format!(
        "// PANopt cockpit Zellij config - generated by `panopt up`, regenerated\n\
         // each run from your own Zellij config. Alt-N and the pane-mode split\n\
         // keys (n/d/r/s) are retargeted to pipe a `panopt:spawn-blank-pane`\n\
         // message to the sidebar plugin, which spawns a blank viewer pane with\n\
         // its own routing slot (and refuses while a sidebar pane is focused, so\n\
         // the gesture can never split the fixed sidebar). CloseFocus, CloseTab,\n\
...
mod tests {
    ...
    #[test]
    fn retarget_pane_mode_splits_funnels_every_split_key_through_the_pipe() {
        // The four stock pane-mode split binds, exactly as the defaults dump.
        let base = "\
            pane {\n        \
        ...
        let (tweaked, changed) = retarget_pane_mode_splits(base);
        assert_eq!(changed, PANE_SPLIT_BINDS.len());
        // No native NewPane survives - every split key now pipes the cockpit's
        // spawn-blank-pane request, which the plugin gates on sidebar focus.
        assert!(!tweaked.contains("NewPane"), "{tweaked}");
        assert_eq!(
            tweaked.matches("\"panopt:spawn-blank-pane\"").count(),
            PANE_SPLIT_BINDS.len()
        );
        // The mode-exit is preserved so the keypress still leaves pane mode.
        assert_eq!(
            tweaked.matches("SwitchToMode \"Normal\"").count(),
            PANE_SPLIT_BINDS.len()
        );
    }

    #[test]
    fn retarget_pane_mode_splits_reports_partial_and_absent_matches() {
        // Only `d` present (a reformatted config): one rewrite, not four.
        let (_, changed) = retarget_pane_mode_splits(
            "    bind \"d\" { NewPane \"Down\"; SwitchToMode \"Normal\"; }\n",
        );
        assert_eq!(changed, 1);
        // Nothing matching at all: zero rewrites, text untouched.
        let (text, none) = retarget_pane_mode_splits("keybinds {}\n");
        assert_eq!(none, 0);
        assert_eq!(text, "keybinds {}\n");
    }
