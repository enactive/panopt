...
# saw, and emits OSC 52 to /dev/tty so the host terminal writes it to
# the system clipboard. Zellij invokes this as its `copy_command`.
LOG={log_path}
...
rc=$?
echo "osc52 rc:    $rc" >> "$LOG" 2>/dev/null
rm -f "$tmp"
"#,
