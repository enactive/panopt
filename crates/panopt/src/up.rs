...
fn ensure_copy_helper_script() -> Result<PathBuf> {
    ...
    let body = format!(
        r#"#!/bin/sh
# Generated by `panopt up`. Reads selection text on stdin, logs what it
# saw, and emits OSC 52. Zellij invokes this as its `copy_command`.
#
# Why stdout rather than /dev/tty: Zellij's `copy_command` subprocess is
# forked by the server with no controlling terminal (`/dev/tty` returns
# ENXIO, rc=1), but its stdout is inherited from Zellij - which Zellij
# writes its render frames to, i.e. the host terminal. Writing OSC 52
# to stdout reaches WezTerm-via-SSH unmolested (interleaved with
# Zellij's render bytes but OSC 52 is non-printing so it's harmless).
# `/dev/tty` is kept as a fallback for the case where stdout is somehow
# redirected (a future Zellij version, or a wrapper script).
LOG={log_path}
...
rc_stdout=$?
echo "osc52 stdout rc: $rc_stdout" >> "$LOG" 2>/dev/null
# Best-effort fallback if stdout was redirected away from the terminal.
if [ "$rc_stdout" -ne 0 ]; then
  printf '\033]52;c;%s\007' "$b" > /dev/tty 2>>"$LOG"
  echo "osc52 tty rc:    $?" >> "$LOG" 2>/dev/null
fi
rm -f "$tmp"
"#,
