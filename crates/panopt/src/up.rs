...
fn ensure_copy_helper_script() -> Result<PathBuf> {
    ...
    // Body kept terse: read text on stdin, base64-encode (Linux `-w 0`
    // suppresses line wrap; macOS `base64` has no `-w` but emits one line
    // by default - fall back to `tr -d '\n'` for safety either way), wrap
    // in OSC 52, write to /dev/tty. Errors silenced because the cockpit
    // shouldn't surface a clipboard write failure as a UI glitch.
    let body = "\
#!/bin/sh
# Generated by `panopt up`. Reads selection text on stdin and emits OSC 52
# to /dev/tty so the host terminal (WezTerm via SSH, iTerm, etc.) writes
# it to the system clipboard. Zellij invokes this as its `copy_command`.
